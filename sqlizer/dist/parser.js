export class SQLParser {
    ...
    parseComment(sql, start, line, column) {
    ...
    parseString(sql, start, line, column) {
    ...
    parseNumber(sql, start, line, column) {
        let end = start;
        let value = '';
        // Parse integer part
    ...
    parseOperator(sql, start, line, column) {
        let end = start;
        let value = sql[start];
    ...
    parseIdentifier(sql, start, line, column) {
        let end = start;
        let value = '';
    ...
    validateSyntax(tokens, errors) {
        ...
        let bracketCount = 0;
        let lastToken = null;
        let clauseOrder = []; // Track the order of clauses
        for (const token of tokens) {
            ...
            if (lastToken) {
                ...
                if (lastToken.type === 'keyword' && token.type === 'keyword') {
                    ...
                    const isAllowed = allowedCombinations.some(([first, second]) => lastUpper === first && upperValue === second);
                    if (!isAllowed) {
                        errors.push({
    ...
    validateClauseOrder(clauseOrder, tokens, errors) {
...
SQLParser.SQL_KEYWORDS = new Set([
...
SQLParser.OPERATORS = new Set([
    '=', '!=', '<>', '<', '>', '<=', '>=', '+', '-', '*', '/', '%', '||'
]);
SQLParser.PUNCTUATION = new Set([
    ',', ';', '(', ')', '.', '[', ']'
]);
