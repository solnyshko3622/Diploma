...
const columnList = () => chain(
  columnExpression,
  many(chain(',', columnExpression)(ast => ast[1]))
)(ast => ({
  type: 'ColumnList',
  columns: [ast[0], ...ast[1]]
}));

const columnExpression = () => chain(
  expression,
  optional(chain('AS', identifier)(ast => ast[1]))
)(ast => ({
...
const orderList = () => chain(
  orderExpression,
  many(chain(',', orderExpression)(ast => ast[1]))
)(ast => ({
  type: 'OrderList',
  orders: [ast[0], ...ast[1]]
}));

const orderExpression = () => chain(
  expression,
  optional(chain(['ASC', 'DESC'])(ast => ast[0]))
)(ast => ({
...
const valueList = () => chain(
  expression,
  many(chain(',', expression)(ast => ast[1]))
)(ast => ({
  type: 'ValueList',
  values: [ast[0], ...ast[1]]
}));
...
const assignmentList = () => chain(
  assignment,
  many(chain(',', assignment)(ast => ast[1]))
)(ast => ({
  type: 'AssignmentList',
...
const columnDefinitionList = () => chain(
  columnDefinition,
  many(chain(',', columnDefinition)(ast => ast[1]))
)(ast => ({
 ...
 */
export function parseSQLWithSyntaxParser(sql: string): SyntaxParserResult {
  try {
    const result = SQLSyntaxParser(sql);
    
    // Convert syntax-parser errors to our format
    const errors: SyntaxParserError[] = [];
 ...
 */
function validateSemantics(ast: any): SyntaxParserError[] {
  const errors: SyntaxParserError[] = [];
 ...
 */
function validateClauseOrder(ast: any, errors: SyntaxParserError[]): void {
